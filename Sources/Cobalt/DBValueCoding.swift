...
internal struct DBValueEncoder {
    ...
    /// Derive a CobaltTableSchema from a model instance.
    ...
    static func deriveSchema<M: CobaltModel>(from instance: M) throws -> CobaltTableSchema {
        let values = try encode(instance)
        let optionalInfo = Self.optionalPropertyInfo(of: instance)

        var columns: [CobaltColumn] = [
        ...
        for key in allKeys.sorted() {
            ...
            columns.append(CobaltColumn(name: key, type: colType, isNullable: isNullable))
        }

        return CobaltTableSchema(name: M.tableName, columns: columns)
    }

    private static func columnType(for value: DBValue) -> CobaltColumnType {
        switch value {
    ...
    private static func optionalPropertyInfo(of value: Any) -> (names: Set<String>, types: [String: CobaltColumnType]) {
        let mirror = Mirror(reflecting: value)
        var names = Set<String>()
        var types = [String: CobaltColumnType]()
