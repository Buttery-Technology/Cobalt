...
/// Configuration for creating a CobaltDatabase instance
public struct CobaltConfiguration: Sendable {
    ...
    /// Returns the default directory for Cobalt databases.
    /// Uses `~/Library/Application Support/Cobalt/`, falling back to the temp directory.
    public static func defaultDirectory() -> String {
        let fm = FileManager.default
        if let appSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            let dir = appSupport.appendingPathComponent("Cobalt").path
    ...
    /// e.g. `databasePath(name: "myapp")` → `~/Library/Application Support/Cobalt/myapp.cobalt`
    public static func databasePath(name: String = "default") -> String {
        let dir = defaultDirectory()
        return (dir as NSString).appendingPathComponent("\(name).cobalt")
    }
    ...
    internal static func resolveEncryptionKey(for dbPath: String) throws -> Data {
        ...
        if fm.fileExists(atPath: keyPath) {
            guard let existing = fm.contents(atPath: keyPath), existing.count == 32 else {
                throw CobaltError.corruptedKeyFile(path: keyPath)
            }
            return existing
        }
