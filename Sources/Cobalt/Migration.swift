...
// MARK: - CobaltDatabase Migration API
...
extension CobaltDatabase {
    ...
    public func migrate(table: String, migrations: [Migration]) async throws {
        guard await tableExists(table) else {
            throw CobaltError.tableNotFound(name: table)
        }
    ...
    private func addColumnToTable(table: String, column: CobaltColumn) async throws {
        ...
        // Update schema
        let newSchema = CobaltTableSchema(name: schema.name, columns: schema.columns + [column])
    ...
    private func dropColumnFromTable(table: String, columnName: String) async throws {
        ...
        if let col = schema.columns.first(where: { $0.name == columnName }), col.isPrimaryKey {
            throw CobaltError.invalidQuery(description: "Cannot drop primary key column '\(columnName)'")
        }
        ...
        let newSchema = CobaltTableSchema(name: schema.name, columns: newColumns)
        try await updateTableSchema(table, schema: newSchema)
    ...
    private func renameColumnInTable(table: String, from: String, to: String) async throws {
        ...
        let newColumns = schema.columns.map { col -> CobaltColumn in
            if col.name == from {
                return CobaltColumn(name: to, type: col.type, isPrimaryKey: col.isPrimaryKey, isNullable: col.isNullable, defaultValue: col.defaultValue, isAutoIncrement: col.isAutoIncrement)
            }
            return col
        }
        let newSchema = CobaltTableSchema(name: schema.name, columns: newColumns)
...
// MARK: - Auto-Migration for CobaltModel
...
extension CobaltDatabase {
    ...
    internal func autoMigrate<M: CobaltModel>(for model: M) async throws {
        ...
        for col in newColumns {
            guard col.isNullable else {
                throw CobaltError.invalidQuery(description: "Auto-migration cannot add non-nullable column '\(col.name)'. Use explicit migrate().")
            }
        }

        // Apply additive migration
        let updatedColumns = existingSchema.columns + newColumns
        let updatedSchema = CobaltTableSchema(name: tableName, columns: updatedColumns)
