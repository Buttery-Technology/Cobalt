...
public final class BufferPoolManager: Sendable {
    ...
    private let stripes: [CobaltLock<StripeState>]
    private let stripeCount: Int
    private let bgWriterConfig: BackgroundWriterConfig
    private let bgWriterTask: CobaltLock<Task<Void, Never>?>
    ...
    public init(capacity: Int, storageManager: StorageManager, stripeCount: Int = 8, bgWriterConfig: BackgroundWriterConfig = BackgroundWriterConfig()) {
        ...
        self.stripes = (0..<max(1, stripeCount)).map { _ in CobaltLock(StripeState()) }
        self.bgWriterConfig = bgWriterConfig
        self.bgWriterTask = CobaltLock(nil)
    }
    ...
    private func backgroundFlushCycle() async {
        ...
        var dirtyPageIDs: [(Int, CobaltLock<StripeState>)] = []
        for s in stripes {
        ...
        // Fsync to ensure flushed pages are durable on disk
        if totalFlushed > 0 {
            try? await storageManager.sync()
        }
    }

    private func stripe(for pageID: Int) -> CobaltLock<StripeState> {
    ...
    /// Combined dirty check + cached page retrieval in a single lock acquisition.
    /// Returns (isDirty, cachedPage?). Avoids two separate stripe lock round-trips
    /// on the PK point lookup hot path.
    public func getDirtyStatusAndCachedPage(pageID: Int) -> (isDirty: Bool, page: DatabasePage?) {
        stripe(for: pageID).withLock { st in
            let dirty = st.dirtyPages.contains(pageID)
            if let page = st.pageCache[pageID] {
                st.referenced.insert(pageID)
                st.hitCount += 1
                return (dirty, page)
    ...
    public func flushAllDirtyPages() async throws {
        ...
        var allDirtyPages: [(Int, DatabasePage, CobaltLock<StripeState>)] = []
        for s in stripes {
