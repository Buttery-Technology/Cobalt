...
public final class StorageManager: Sendable {
    ...
    private let mmapState: CobaltRWLock<MmapState>
    ...
    public init(databasePath: String, encryptionProvider: EncryptionProvider? = nil) throws {
        ...
        self.pageSize = CobaltConstants.PAGE_SIZE
        self.diskPageSize = encryptionProvider != nil ? CobaltConstants.ENCRYPTED_PAGE_SIZE : CobaltConstants.PAGE_SIZE
        ...
        guard fileFD >= 0 else {
            throw CobaltError.fileOpenError(description: "Failed to open database at \(databasePath)")
        }
        ...
        if encryptionProvider == nil {
            ...
            if fileSize > 0 {
                ...
                if ptr != MAP_FAILED {
                    self.mmapState = CobaltRWLock(MmapState(baseAddress: UnsafeRawPointer(ptr), mappedLength: length))
                } else {
                    self.mmapState = CobaltRWLock(MmapState())
                }
            } else {
                self.mmapState = CobaltRWLock(MmapState())
            }
    ...
    private func requireFD() throws -> Int32 {
        ...
        guard let fileFD, fileFD >= 0 else {
            throw CobaltError.databaseClosed
        }
        return fileFD
    }
    ...
    public func writePage(_ page: inout DatabasePage, alreadySerialized: Bool = false) throws {
        ...
        // Compute CRC32 over bytes 0..<PAGE_USABLE_END, store in last 4 bytes
        let checksumOffset = CobaltConstants.PAGE_USABLE_END
        page.data.withUnsafeMutableBytes { buf in
            let crc = CRC32.checksum(UnsafeRawBufferPointer(buf), count: checksumOffset)
            buf.storeBytes(of: crc, toByteOffset: checksumOffset, as: UInt32.self)
        }
        ...
        try dataToWrite.withUnsafeBytes { rawBuf in
            ...
            while written < dataToWrite.count {
                ...
                if result < 0 {
                    throw CobaltError.pageWriteError(description: "pwrite failed for page \(page.pageID): errno \(errno)")
                }
                written += result
            }
        }
    ...
    public func readPage(pageID: Int) throws -> DatabasePage {
        ...
        guard bytesRead == diskPageSize else {
            throw CobaltError.pageReadError(description: "Failed to read page \(pageID)")
        }
        ...
        // Verify CRC32 checksum (skip if stored checksum is 0 for backward compat)
        try pageData.withUnsafeBytes { buf in
            let checksumOffset = CobaltConstants.PAGE_USABLE_END
            let storedCRC = buf.loadUnaligned(fromByteOffset: checksumOffset, as: UInt32.self)
            if storedCRC != 0 {
                let computedCRC = CRC32.checksum(buf, count: checksumOffset)
                if computedCRC != storedCRC {
                    throw CobaltError.corruptPage(pageID: pageID)
                }
            }
        }
    ...
    public func createNewPage() throws -> DatabasePage {
        ...
        guard currentSize >= 0 else {
            throw CobaltError.pageWriteError(description: "lseek failed: errno \(errno)")
        }
        ...
        var page = DatabasePage(
            ...
            nextPageID: 0,
            recordCount: 0,
            freeSpaceOffset: CobaltConstants.PAGE_USABLE_END,
            flags: 0,
            data: Data(count: pageSize)
        )
        try page.saveRecords()

        // Compute CRC32 checksum before writing
        let checksumOffset = CobaltConstants.PAGE_USABLE_END
        page.data.withUnsafeMutableBytes { buf in
            let crc = CRC32.checksum(UnsafeRawBufferPointer(buf), count: checksumOffset)
            buf.storeBytes(of: crc, toByteOffset: checksumOffset, as: UInt32.self)
        ...
        try dataToWrite.withUnsafeBytes { rawBuf in
            ...
            while written < dataToWrite.count {
                let result = pwrite(fileFD, baseAddr + written, dataToWrite.count - written, offset + off_t(written))
                if result < 0 {
                    throw CobaltError.pageWriteError(description: "pwrite failed for new page \(newPageID): errno \(errno)")
                }
                written += result
            }
        }
    ...
    public func createNewPages(count: Int) throws -> [DatabasePage] {
        ...
        if ftruncate(fileFD, newSize) < 0 {
            throw CobaltError.pageWriteError(description: "ftruncate failed: errno \(errno)")
        }

        var pages = [DatabasePage]()
        pages.reserveCapacity(count)
        let usableEnd = CobaltConstants.PAGE_USABLE_END
        for i in 0..<count {
            ...
            pageData.withUnsafeMutableBytes { buf in
                ...
                buf.storeBytes(of: Int32(usableEnd), toByteOffset: 20, as: Int32.self)
                // Compute and store CRC32 checksum
                let crc = CRC32.checksum(UnsafeRawBufferPointer(buf), count: usableEnd)
                buf.storeBytes(of: crc, toByteOffset: usableEnd, as: UInt32.self)
            }
            pages.append(DatabasePage(
                pageID: firstPageID + i,
                nextPageID: 0,
                recordCount: 0,
                freeSpaceOffset: usableEnd,
                flags: 0,
                data: pageData
            ))
        }
    ...
    public func readPages(pageIDs: [Int]) throws -> [DatabasePage] {
        ...
        func readRun(start: Int, count: Int) throws {
            ...
            for i in 0..<count {
                ...
                // Verify CRC32 checksum (skip if stored checksum is 0 for backward compat)
                let currentPageID = start + i
                try pageData.withUnsafeBytes { buf in
                    let checksumOff = CobaltConstants.PAGE_USABLE_END
                    let storedCRC = buf.loadUnaligned(fromByteOffset: checksumOff, as: UInt32.self)
                    if storedCRC != 0 {
                        let computedCRC = CRC32.checksum(buf, count: checksumOff)
                        if computedCRC != storedCRC {
                ...
                var page = DatabasePage(pageID: currentPageID, data: pageData)
                page.loadRecords()
                pages.append(page)
            }
        }
    ...
    public func readPageMmap(pageID: Int) -> DatabasePage? {
        mmapState.withReadLock { state in
            ...
            // Verify CRC32 checksum directly from mmap'd memory
            let ptr = base + offset
            let checksumOff = CobaltConstants.PAGE_USABLE_END
            let storedCRC = ptr.loadUnaligned(fromByteOffset: checksumOff, as: UInt32.self)
            if storedCRC != 0 {
                let computedCRC = CRC32.checksum(
                    UnsafeRawBufferPointer(start: ptr, count: checksumOff), count: checksumOff)
                if computedCRC != storedCRC { return nil }
            }

            let pageData = Data(bytes: ptr, count: pageSize)
    ...
    public func readRecordMmap(pageID: Int, recordID: UInt64) -> Data? {
        mmapState.withReadLock { state in
            ...
            // Scan slot directory backwards (recently added records are at the end,
            // and PK lookups typically target the most recently inserted record for that key).
            // Slot directory starts at offset 28, 6 bytes per slot.
            var slotPos = 28 + (recordCount - 1) * 6
    ...
    public func sync() throws {
        let fileFD = try requireFD()
        if fsync(fileFD) < 0 {
            throw CobaltError.pageWriteError(description: "fsync failed: errno \(errno)")
        }
    }
    ...
    public func totalPageCount() throws -> Int {
        ...
        guard size >= 0 else {
            throw CobaltError.pageReadError(description: "lseek failed: errno \(errno)")
        }
