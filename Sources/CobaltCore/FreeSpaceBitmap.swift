...
public final class FreeSpaceBitmap: Sendable {
    ...
    private let state: CobaltRWLock<State>

    /// Maximum pages tracked by a single 8KB bitmap page (2 bits each)
    private static let pagesPerBitmapPage = (CobaltConstants.PAGE_USABLE_END - CobaltConstants.PAGE_HEADER_SIZE - 12) * 4
    ...
    public init() {
        self.state = CobaltRWLock(State(bitmap: Data(count: Self.initialBitmapSize), pageID: 0, dirty: false))
    }
...
public final class CobaltLock<Value>: @unchecked Sendable {
...
public final class CobaltRWLock<Value>: @unchecked Sendable {
    private var value: Value
