...
/// Result of WAL crash recovery
public struct RecoveryResult: Sendable {
    public let transactionsReplayed: Int
    public let pagesReplayed: Int
    public let corruptRecordsSkipped: Int
    public let walTruncated: Bool
}
...
public actor WriteAheadLog: Sendable {
    ...
    /// Cached timestamp to avoid repeated Date()/timeIntervalSince1970 syscalls.
    /// Refreshes on transaction begin or when >1ms has elapsed.
    private var cachedTimestampSeconds: UInt64 = 0
    private var cachedTimestampNanos: UInt64 = 0

    /// Get current timestamp, using cache when <1ms has elapsed.
    private func currentTimestamp() -> UInt64 {
        let now = DispatchTime.now().uptimeNanoseconds
        if now &- cachedTimestampNanos > 1_000_000 {
            cachedTimestampSeconds = UInt64(Date().timeIntervalSince1970)
            cachedTimestampNanos = now
        }
        return cachedTimestampSeconds
    }

    /// Force-refresh the cached timestamp (call on transaction begin).
    private func refreshTimestamp() -> UInt64 {
        cachedTimestampSeconds = UInt64(Date().timeIntervalSince1970)
        cachedTimestampNanos = DispatchTime.now().uptimeNanoseconds
        return cachedTimestampSeconds
    }
    ...
    public func logTransactionBegin(txID: UInt64, isolationLevel: IsolationLevel) throws {
        ...
        let timestamp = refreshTimestamp()
    ...
    public func recoverPage(pageID: Int) throws -> DatabasePage {
        ...
        throw CobaltError.pageNotInLog
    }
    ...
    private func writeLogRecord(_ data: Data) throws {
        guard let fh = logFileHandle else {
            throw CobaltError.walWriteError(description: "WAL file handle is closed")
        }
    ...
    private func getAllLogRecords() throws -> [LogRecord] {
        guard let handle = logFileHandle else {
            throw CobaltError.logReadError
        }
        ...
        while pos + 8 <= allData.count {
            ...
            guard computedCRC == storedCRC else {
                // CRC mismatch — skip this record but don't silently ignore it.
                // For getAllLogRecords (runtime reads), just skip; corruption handling
                // is done in parsePersistedRecords during recovery.
                pos += dataLength
                continue
            }
    ...
    /// Replay committed transactions from the WAL to the data file.
    ///
    /// This must be called during startup after the WAL is opened but before normal
    /// operation begins. It scans all persisted WAL records, identifies transactions
    /// that have a commit record, replays their after-images (page writes) to the
    /// data file, discards uncommitted transactions, and truncates the WAL.
    @discardableResult
    public func recover() async throws -> RecoveryResult {
        let (records, corruptRecordsSkipped) = try parsePersistedRecords()

        // Nothing to recover if the WAL is empty
        guard !records.isEmpty else {
            return RecoveryResult(transactionsReplayed: 0, pagesReplayed: 0, corruptRecordsSkipped: corruptRecordsSkipped, walTruncated: false)
        }

        // Identify committed transaction IDs (those with a commit record)
        var committedTxIDs = Set<UInt64>()
        for record in records {
            if record.type == .transactionCommit {
                committedTxIDs.insert(record.transactionID)
            }
        }

        // Nothing to replay if no transactions committed
        guard !committedTxIDs.isEmpty else {
            try truncate()
            return RecoveryResult(transactionsReplayed: 0, pagesReplayed: 0, corruptRecordsSkipped: corruptRecordsSkipped, walTruncated: true)
        }

        // Collect the last after-image for each page from committed transactions.
        // Process records in LSN order so later writes overwrite earlier ones.
        let sortedRecords = records.sorted { $0.lsn < $1.lsn }
        var pagesToReplay: [Int: Data] = [:]  // pageID -> latest after-image data

        for record in sortedRecords {
            guard committedTxIDs.contains(record.transactionID) else { continue }

            switch record.type {
            case .pageAfterUpdate:
                if case let .pageImage(pageID, _, pageData) = record.content {
                    pagesToReplay[pageID] = pageData
                }
            case .pageAfterDelta:
                if case let .pageDelta(pageID, _, delta) = record.content {
                    // Read the current page from disk and apply the delta
                    let currentPage = try await storageManager.readPage(pageID: pageID)
                    if let newData = Self.applyDelta(to: currentPage.data, delta: delta) {
                        pagesToReplay[pageID] = newData
                    }
                }
            default:
                break
        ...
        // Replay after-images to the data file
        for (pageID, pageData) in pagesToReplay {
            var page = DatabasePage(pageID: pageID, data: pageData)
            page.loadRecords()
            try await storageManager.writePage(&page)
        }

        // Sync the data file to ensure replayed pages are durable
        try await storageManager.sync()

        // Truncate the WAL now that all committed data is in the data file
        try truncate()

        let result = RecoveryResult(transactionsReplayed: committedTxIDs.count, pagesReplayed: pagesToReplay.count, corruptRecordsSkipped: corruptRecordsSkipped, walTruncated: true)
        if corruptRecordsSkipped > 0 {
            print("[CobaltCore WAL] Recovery complete: \(result.transactionsReplayed) transaction(s) replayed, \(result.pagesReplayed) page(s) replayed, \(corruptRecordsSkipped) corrupt record(s) skipped")
        }
        return result
    }
    ...
    private func recoverNextLSN() throws {
        let (records, _) = try parsePersistedRecords()
    ...
    /// Returns the parsed records and the number of corrupt records skipped.
    private func parsePersistedRecords() throws -> (records: [LogRecord], corruptRecordsSkipped: Int) {
        guard let handle = logFileHandle else {
            throw CobaltError.logReadError
        }
        ...
        // First pass: parse all record frames, marking each as valid or corrupt
        struct RecordFrame {
            let pos: Int
            let dataLength: Int
            let isValid: Bool
        ...
        while pos + 8 <= allData.count {
            ...
            if computedCRC != storedCRC {
                frames.append(RecordFrame(pos: pos, dataLength: dataLength, isValid: false, record: nil))
                pos += dataLength
                continue
            }
            ...
            let recordData: Data
            if let provider = encryptionProvider {
                do { recordData = try provider.decrypt(payload) } catch {
                    frames.append(RecordFrame(pos: pos, dataLength: dataLength, isValid: false, record: nil))
                    pos += dataLength
                    continue
                }
            ...
            let record = parseLogRecord(recordData)
            frames.append(RecordFrame(pos: pos, dataLength: dataLength, isValid: true, record: record))
            pos += dataLength
        }

        // Second pass: distinguish torn tail writes from mid-WAL corruption.
        // If a corrupt record has no valid records after it, it's a torn tail write — stop there.
        // If valid records follow a corrupt record, it's mid-WAL corruption — skip but count it.
        var records: [LogRecord] = []
        var corruptRecordsSkipped = 0

        for i in 0..<frames.count {
            let frame = frames[i]
            if frame.isValid {
                if let record = frame.record {
                    records.append(record)
                }
            } else {
                // Check if any valid records follow this corrupt one
                let hasValidAfter = frames[(i + 1)...].contains { $0.isValid }
                if hasValidAfter {
                    // Mid-WAL corruption: skip but count
                    corruptRecordsSkipped += 1
                } else {
                    // Torn tail write: no valid records follow, stop parsing
                    // Count this and any remaining corrupt frames
                    let remainingCorrupt = frames[i...].filter { !$0.isValid }.count
                    if remainingCorrupt > 0 {
                        // Torn tail writes are expected during crash recovery — not counted as skipped corruption
                    }
                    break
                }
            }
        ...
        if corruptRecordsSkipped > 0 {
            print("[CobaltCore WAL] WARNING: \(corruptRecordsSkipped) corrupt record(s) skipped during WAL recovery (mid-WAL corruption detected)")
        }

        return (records, corruptRecordsSkipped)
    }
    ...
    public func close() throws {
        // Drain all pending commit continuations so callers don't deadlock
        let batch = pendingCommits
        pendingCommits = []
        for item in batch {
            item.continuation.resume(throwing: CobaltError.databaseClosed)
        }
