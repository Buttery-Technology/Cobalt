...
/// Protocol that CobaltIndex implements to provide index operations to the storage engine
public protocol IndexHook: Sendable {
    ...
    func removeFromIndexesBatchRaw(records: [(id: UInt64, data: Data)], tableName: String, schema: CobaltTableSchema) async throws
}
...
public final class StorageEngine: @unchecked Sendable {
    ...
    private let _state: CobaltLock<MutableState>
    ...
    public init(databasePath: String, bufferPoolCapacity: Int = 1000, encryptionProvider: EncryptionProvider? = nil, bufferPoolStripeCount: Int = 8, bgWriterIntervalMs: Int = 100) async throws {
        ...
        // Replay any committed transactions from a previous crash before normal operation
        let recoveryResult = try await logManager.recover()
        if recoveryResult.transactionsReplayed > 0 || recoveryResult.corruptRecordsSkipped > 0 {
            print("[CobaltCore] WAL recovery: \(recoveryResult.transactionsReplayed) transaction(s) replayed, \(recoveryResult.pagesReplayed) page(s) replayed, \(recoveryResult.corruptRecordsSkipped) corrupt record(s) skipped, WAL truncated: \(recoveryResult.walTruncated)")
        }
        ...
        self._state = CobaltLock(MutableState())
        ...
        let metaPage = try await sm.readPage(pageID: CobaltConstants.SYSTEM_PAGE_DB_METADATA)
    ...
    public func insertRecord(_ record: Record, tableName: String, row: Row? = nil, transactionContext: TransactionContext? = nil) async throws -> UInt64 {
        ...
        let maxInlineSize = CobaltConstants.MAX_INLINE_RECORD_SIZE
        ...
        if !page.addRecord(record) {
            ...
            if !newPage.addRecord(record) {
                throw CobaltError.recordTooLarge(size: serializedSize)
            }
    ...
    public func bulkInsertRecordsBatched(_ records: [Record], tableName: String, transactionContext: TransactionContext? = nil) async throws -> Int {
        ...
        let maxInlineSize = CobaltConstants.MAX_INLINE_RECORD_SIZE
        let pageCapacity = CobaltConstants.PAGE_USABLE_END - CobaltConstants.PAGE_HEADER_SIZE
        var inserted = 0
        ...
        // Pre-estimate pages needed and pre-size bitmap updates array
        var totalDataSize = 0
        for record in records {
            totalDataSize += record.serializedSize + CobaltConstants.SLOT_SIZE
        }
        let estimatedPages = max(1, (totalDataSize + pageCapacity - 1) / pageCapacity)
        var bitmapUpdates: [(pageID: Int, category: SpaceCategory)] = []
        bitmapUpdates.reserveCapacity(estimatedPages)

        // Pre-allocate extent reserve if we'll need many pages beyond the current extent
        if transactionContext == nil && estimatedPages > extentSize {
            let currentReserve = _state.withLock { $0.extentReserve[tableName]?.count ?? 0 }
            let pagesNeeded = estimatedPages - currentReserve
            if pagesNeeded > extentSize {
                // Pre-allocate a large batch of pages to avoid repeated extent allocations
                let batchSize = pagesNeeded
                let preAllocated = try storageManager.createNewPages(count: batchSize)
                for page in preAllocated {
                    bufferPoolManager.cachePageSync(page)
                    bufferPoolManager.markDirty(pageID: page.pageID)
                }
                _state.withLock { s in
                    var reserve = s.extentReserve[tableName, default: []]
                    reserve.append(contentsOf: preAllocated)
                    s.extentReserve[tableName] = reserve
                }
            }
        }
    ...
    public func deleteRecord(id: UInt64, tableName: String, transactionContext: TransactionContext? = nil, knownPageID: Int? = nil) async throws {
        ...
        if !page.deleteRecord(id: id) {
            throw CobaltError.recordNotFound(id: id)
        }
    ...
    public func deleteRecordsBatch(_ records: [(id: UInt64, pageID: Int, row: Row?)], tableName: String, transactionContext: TransactionContext? = nil) async throws {
        ...
        var deferredPages: [DatabasePage] = []  // collect for batch save
        ...
        for pageID in byPage.keys.sorted() {
            ...
            // Build set of IDs to delete on this page for batch deletion
            ...
            // Batch delete all records on this page in one call
        ...
        // Batch save deferred pages — single pass instead of per-page saves
    ...
    public func scanTable(_ tableName: String, transactionContext: TransactionContext? = nil, neededColumns: Set<String>? = nil) async throws -> [(Record, Row)] {
        ...
        while currentPageID != 0 {
            guard visited.insert(currentPageID).inserted else {
                throw CobaltError.corruptPage(pageID: currentPageID)
            }
    ...
    public func getRecordByIDSync(_ id: UInt64, tableName: String) -> Row? {
        ...
        // Single lock acquisition: get dirty status + cached page together
        let (isDirty, cachedPage) = bufferPoolManager.getDirtyStatusAndCachedPage(pageID: pageID)

        // Fast path: mmap for clean pages
        if storageManager.mmapAvailable && !isDirty {
        ...
        // Fallback: use the cached page we already retrieved (no second lock)
    ...
    /// Schema-preloaded variant: avoids the TableRegistry lock when the caller already has the schema.
    /// Used by QueryExecutor.executeSelectSync to eliminate one lock acquisition from the hot path.
    public func getRecordByIDSyncWithSchema(_ id: UInt64, tableName: String, schema: CobaltTableSchema?) -> Row? {
        let pageID: Int? = _state.withLock { $0.ridPageMap[tableName]?[id] }
        guard let pageID else { return nil }
        ...
        if storageManager.mmapAvailable && !isDirty {
            if let recordData = storageManager.readRecordMmap(pageID: pageID, recordID: id) {
                return Row.fromBytesAuto(recordData, schema: schema)
            }
        }
    ...
    public func getTableSchema(_ name: String) -> CobaltTableSchema? {
        tableRegistry.getTableInfo(name: name)?.schema
    }

    /// Update a table's schema in the registry (does not modify existing rows)
    public func updateTableSchema(_ name: String, schema: CobaltTableSchema) async throws {
        guard var info = tableRegistry.getTableInfo(name: name) else {
            throw CobaltError.tableNotFound(name: name)
        }
        info.schema = schema
        tableRegistry.updateTableInfo(info)
    ...
    private func findTargetPageForInsert(tableInfo: TableInfo, recordSize: Int) async throws -> Int {
        let requiredSpace = recordSize + CobaltConstants.SLOT_SIZE
    ...
    private func insertOverflowRecord(_ record: Record, tableName: String, transactionContext: TransactionContext? = nil) async throws -> Record {
        ...
        let overflowPayload = CobaltConstants.OVERFLOW_PAGE_PAYLOAD
        ...
        let maxInlineData = CobaltConstants.PAGE_USABLE_END - CobaltConstants.PAGE_HEADER_SIZE - CobaltConstants.SLOT_SIZE - 12 - 9
        ...
        for i in 0..<overflowPages.count {
            ...
            var pageData = Data(count: CobaltConstants.PAGE_SIZE)
            ...
            pos += 4
            var fso = Int32(CobaltConstants.PAGE_USABLE_END)
    ...
    public func reassembleOverflowRecord(_ record: Record) async throws -> Record {
        ...
        while currentOverflowPageID != 0 {
            guard visited.insert(currentOverflowPageID).inserted else {
                prefetchTask?.cancel()
                throw CobaltError.corruptPage(pageID: currentOverflowPageID)
            }

            let page = try await storageManager.readPage(pageID: currentOverflowPageID)
            let headerSize = CobaltConstants.PAGE_HEADER_SIZE
            ...
            let dataEnd = CobaltConstants.PAGE_USABLE_END
    ...
    public func saveIndexRegistry(_ data: Data) async throws {
        let maxRecordDataSize = CobaltConstants.PAGE_USABLE_END - CobaltConstants.PAGE_HEADER_SIZE - CobaltConstants.SLOT_SIZE - 12 - 1 // 12 = record header (id + length), -1 for strict >= check in addRecord
    ...
    private func saveDBMetadata() async throws {
        ...
        var metaPage = try await storageManager.readPage(pageID: CobaltConstants.SYSTEM_PAGE_DB_METADATA)
        metaPage.records = [record]
        metaPage.recordCount = 1
        metaPage.freeSpaceOffset = CobaltConstants.PAGE_USABLE_END - record.serializedSize
