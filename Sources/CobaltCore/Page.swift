...
public struct DatabasePage: Sendable {
    ...
    public init(
        ...
        nextPageID: Int = 0,
        recordCount: Int = 0,
        freeSpaceOffset: Int = CobaltConstants.PAGE_USABLE_END,
        flags: UInt32 = 0,
        data: Data = Data(count: CobaltConstants.PAGE_SIZE)
    ) {
        self.pageID = pageID
    ...
    public mutating func loadRecords() {
        ...
        let maxSlots = (CobaltConstants.PAGE_SIZE - CobaltConstants.PAGE_HEADER_SIZE) / CobaltConstants.SLOT_SIZE
        if recordCount < 0 || recordCount > maxSlots {
            recordCount = 0
        }

        // Validate freeSpaceOffset bounds — corrupted values can cause out-of-bounds access
        // Accept PAGE_SIZE for backward compat with pre-checksum pages
        if freeSpaceOffset < CobaltConstants.PAGE_HEADER_SIZE || freeSpaceOffset > CobaltConstants.PAGE_SIZE {
        ...
        for _ in 0..<recordCount {
            guard position + CobaltConstants.SLOT_SIZE <= data.count else { break }
    ...
    public mutating func saveRecords() throws {
        ...
        let slotSize = CobaltConstants.SLOT_SIZE

        var dataPosition = CobaltConstants.PAGE_USABLE_END
        ...
        if overflowed { throw CobaltError.pageOverflow }
    ...
    public mutating func addRecord(_ record: Record, knownSerializedSize: Int) -> Bool {
        ...
        let headerEnd = CobaltConstants.PAGE_HEADER_SIZE + ((recordCount + 1) * slotSize)
    ...
    public mutating func replaceRecord(id: UInt64, with newRecord: Record) -> Bool {
        ...
        let headerEnd = CobaltConstants.PAGE_HEADER_SIZE + (recordCount * CobaltConstants.SLOT_SIZE)
    ...
    public mutating func deleteRecordsAndPatch(ids: Set<UInt64>) -> Int {
        ...
        data.withUnsafeMutableBytes { buf in
            ...
            var pos = CobaltConstants.PAGE_HEADER_SIZE
            ...
            let oldSlotsEnd = CobaltConstants.PAGE_HEADER_SIZE +
                (currentRecordCount + deletedCount) * CobaltConstants.SLOT_SIZE
            if pos < oldSlotsEnd {
