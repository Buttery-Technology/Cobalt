...
/// Each B-tree node is serialized and stored in a CobaltCore page.
...
public final class PageBackedNodeStore: @unchecked Sendable {
    ...
    private let state: CobaltLock<State>
    ...
    public func flushDirtyNodes() async throws {
        ...
        for (_, node, pageID) in nodesToFlush {
            ...
            let maxRecordSize = CobaltConstants.PAGE_USABLE_END - CobaltConstants.PAGE_HEADER_SIZE - CobaltConstants.SLOT_SIZE
            guard recordSize <= maxRecordSize else {
                throw CobaltError.pageOverflow
            }
