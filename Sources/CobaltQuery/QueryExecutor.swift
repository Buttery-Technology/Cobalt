...
public final class QueryExecutor: @unchecked Sendable {
    ...
    private let _cache: CobaltLock<CacheState>
    ...
    public init(storageEngine: StorageEngine, indexManager: IndexManager, tableRegistry: TableRegistry, costWeights: CostModelWeights = .default) {
        ...
        self._cache = CobaltLock(CacheState())
    }
    ...
    /// Fully synchronous point lookup: index → B-tree → mmap record read.
    ...
    /// Optimizations over the general path:
    /// - Skips bloom filter for PK lookups (PK values are always in the index)
    /// - Pre-loads schema once, passes it through to avoid redundant TableRegistry lock
    /// - Uses combined dirty+cached page check (single buffer pool lock)
    public func executeSelectSync(from table: String, columns: [String]? = nil, where condition: WhereCondition, limit: Int = 1) -> [Row]? {
        ...
        // Pre-load schema once for the entire lookup (avoids redundant lock in getRecordByIDSync)
        let schema = storageEngine.getTableSchema(table)

        // Skip bloom filter for PK lookups: PK values are always indexed
        let isPK = schema?.primaryKeyColumn?.name == column
        let ridResult: UInt64??
        if isPK {
            ridResult = index.searchCachedFirstRIDNoBloom(key: value)
        } else {
            ridResult = index.searchCachedFirstRID(key: value)
        }
        guard let ridResult else { return nil } // cache miss
        guard let rid = ridResult else { return [] } // definitive miss

        // Use schema-preloaded variant to avoid second TableRegistry lock
        guard let syncRow = storageEngine.getRecordByIDSyncWithSchema(rid, tableName: table, schema: schema) else { return nil }
        if let columns = columns, !columns.isEmpty {
            var projected = [String: DBValue](minimumCapacity: columns.count)
            for c in columns { projected[c] = syncRow.values[c] ?? .null }
    ...
    public func executeSelect(from table: String, columns: [String]? = nil, where condition: WhereCondition? = nil, modifiers: QueryModifiers? = nil, transactionContext: TransactionContext? = nil) async throws -> [Row] {
        ...
        // Pre-loads schema once and skips bloom filter for PK lookups.
           ...
           (mods.orderBy == nil || mods.orderBy!.isEmpty) {
            if case .equals(let column, let value) = cond, value != .null {
                if let index = indexManager.getIndex(tableName: table, columnName: column) {
                    let schema = storageEngine.getTableSchema(table)
                    let isPK = schema?.primaryKeyColumn?.name == column
                    let ridResult: UInt64?? = isPK
                        ? index.searchCachedFirstRIDNoBloom(key: value)
                        : index.searchCachedFirstRID(key: value)
                    if let ridResult {
                        ...
                        if let syncRow = storageEngine.getRecordByIDSyncWithSchema(rid, tableName: table, schema: schema) {
                            if let columns = columns, !columns.isEmpty {
                                var projected = [String: DBValue](minimumCapacity: columns.count)
                                for c in columns { projected[c] = syncRow.values[c] ?? .null }
                                return [Row(values: projected)]
    ...
    public func executeJoin(from table: String, joins: [JoinClause], columns: [String]? = nil, where condition: WhereCondition? = nil, modifiers: QueryModifiers? = nil, transactionContext: TransactionContext? = nil) async throws -> [Row] {
           ...
           !mods.distinct {
            ...
            if let rightIndex = rightIndex {
                ...
                if let orderBy = mods.orderBy, !orderBy.isEmpty {
                    ...
                    let totalNeeded = limit + (mods.offset ?? 0)
                    if totalNeeded < result.count {
                        result = topN(result, n: totalNeeded, by: comparator)
                    } else {
        ...
        if let mods = modifiers {
            ...
            if let orderBy = mods.orderBy, !orderBy.isEmpty {
                ...
                let totalNeeded = (mods.limit ?? Int.max) + (mods.offset ?? 0)
                if totalNeeded < result.count && totalNeeded < Int.max {
                    result = topN(result, n: totalNeeded, by: comparator)
                } else {
                    result.sort(by: comparator)
                }
    ...
    public func executeGroupBy(from table: String, select expressions: [SelectExpression], where condition: WhereCondition? = nil, groupBy: GroupByClause, modifiers: QueryModifiers? = nil, transactionContext: TransactionContext? = nil) async throws -> [Row] {
        ...
        if let mods = modifiers {
            ...
            if let orderBy = mods.orderBy, !orderBy.isEmpty {
                ...
                let totalNeeded = (mods.limit ?? Int.max) + (mods.offset ?? 0)
                if totalNeeded < resultRows.count && totalNeeded < Int.max {
                    resultRows = topN(resultRows, n: totalNeeded, by: comparator)
                } else {
                    resultRows.sort(by: comparator)
                }
            }
           ...
           let pkColumn = schema.primaryKeyColumn {
            if let pkValue = row.values[pkColumn.name], pkValue != .null {
                ...
                if let indexed = try await indexManager.attemptIndexLookup(
                    ...
                    throw CobaltError.primaryKeyViolation
                ...
                throw CobaltError.notNullConstraintViolation(column: pkColumn.name)
            }
        }

        let schema = storageEngine.getTableSchema(table)
        let rowData = schema != nil ? Row.serializeDirectly(values: row.values, schema: schema!) : row.toBytes()
    ...
    public func executeBulkInsert(into table: String, rows: [Row], transactionContext: TransactionContext? = nil) async throws {
        ...
        recordIDs.reserveCapacity(rowCount)
        for _ in rows { recordIDs.append(generateRecordID()) }

        // Parallel serialization: build Record objects directly (no intermediate Data? array)
        // Use a single contiguous array for both serialized records and index pairs
        var insertedPairs = [(Record, Row)](repeating: (Record(id: 0, data: Data()), Row(values: [:])), count: rowCount)
        if let schema = bulkSchema {
            insertedPairs.withUnsafeMutableBufferPointer { buf in
                DispatchQueue.concurrentPerform(iterations: rowCount) { i in
                    let data = Row.serializeDirectly(values: rows[i].values, schema: schema)
                    buf[i] = (Record(id: recordIDs[i], data: data), rows[i])
                }
            }
        } else {
            for i in 0..<rowCount {
                insertedPairs[i] = (Record(id: recordIDs[i], data: rows[i].toBytes()), rows[i])
            }
        }

        // Extract records for bulk insert (shares Record structs, no data copy)
        var serializedRecords = [Record]()
        serializedRecords.reserveCapacity(rowCount)
        for i in 0..<rowCount {
            serializedRecords.append(insertedPairs[i].0)
        }
    ...
    public func executeAggregate(from table: String, _ function: AggregateFunction, where condition: WhereCondition? = nil, transactionContext: TransactionContext? = nil) async throws -> DBValue {
        ...
        // COUNT(col) without WHERE on a NOT NULL column → same as COUNT(*)
        if case .count(let col) = function, let colName = col, condition == nil,
           let schema = storageEngine.getTableSchema(table),
           let colDef = schema.columns.first(where: { $0.name == colName }),
           !colDef.isNullable {
            let count = planner.registry.getTableInfo(name: table)?.recordCount ?? 0
            return .integer(Int64(count))
        }
        ...
        // Specialized integer accumulators for AVG/MIN/MAX — avoid Int64→Double conversion per-row
        var avgIntSum: Int64 = 0; var avgAllInts = true; var avgIntOverflow = false
        var minVal: DBValue = .null; var maxVal: DBValue = .null
        var minInt: Int64 = Int64.max; var maxInt: Int64 = Int64.min; var minMaxAllInts = true
        ...
        func processRecordData(_ recordData: Data) {
            ...
            if let colIdx = aggColumnIndex {
                if let rawValue = Row.extractColumnValue(from: recordData, columnIndex: colIdx) {
                    ...
                    case .avg:
                        ...
                        case .integer(let v):
                            avgCount += 1
                            if avgAllInts && !avgIntOverflow {
                                let (r, o) = avgIntSum.addingReportingOverflow(v)
                                if o { avgIntOverflow = true; avgSum += Double(avgIntSum) + Double(v) } else { avgIntSum = r }
                            } else { avgSum += Double(v) }
                        case .double(let v): avgAllInts = false; avgSum += v; avgCount += 1
                        default: break
                        }
                    case .min:
                        switch rawValue {
                        case .integer(let v):
                            if minMaxAllInts { if v < minInt { minInt = v }; if minVal == .null { minVal = rawValue } }
                            else { if minVal == .null || rawValue < minVal { minVal = rawValue } }
                        case .null: break
                        default: minMaxAllInts = false; if minVal == .null || rawValue < minVal { minVal = rawValue }
                        }
                    case .max:
                        switch rawValue {
                        case .integer(let v):
                            if minMaxAllInts { if v > maxInt { maxInt = v }; if maxVal == .null { maxVal = rawValue } }
                            else { if maxVal == .null || rawValue > maxVal { maxVal = rawValue } }
                        case .null: break
                        default: minMaxAllInts = false; if maxVal == .null || rawValue > maxVal { maxVal = rawValue }
                        }
                    }
                    return
                }
            ...
            case .min(let col):
                if let v = row.values[col] {
                    switch v {
                    case .integer(let iv):
                        if minMaxAllInts { if iv < minInt { minInt = iv }; if minVal == .null { minVal = v } }
                        else { if minVal == .null || v < minVal { minVal = v } }
                    case .null: break
                    default: minMaxAllInts = false; if minVal == .null || v < minVal { minVal = v }
                    }
                }
            case .max(let col):
                if let v = row.values[col] {
                    switch v {
                    case .integer(let iv):
                        if minMaxAllInts { if iv > maxInt { maxInt = iv }; if maxVal == .null { maxVal = v } }
                        else { if maxVal == .null || v > maxVal { maxVal = v } }
                    case .null: break
                    default: minMaxAllInts = false; if maxVal == .null || v > maxVal { maxVal = v }
                    }
                }
            }
        }
        ...
        func processRecordUnsafe(_ ptr: UnsafeRawBufferPointer) {
            if let colIdx = aggColumnIndex {
                if condition == nil {
                ...
                } else if let cond = condition, let colMap = aggColMap {
                    // Condition present — try unsafe lazy evaluation to avoid Data copy
                    if let lazyResult = evaluateConditionLazyUnsafe(cond, ptr: ptr, columnIndexMap: colMap) {
                        if !lazyResult { return }
                        // Condition passed; extract + aggregate without Data allocation
                        if case .count(nil) = function {
                            count += 1
                            return
                        }
                        if let rawValue = Row.extractColumnValueUnsafe(from: ptr, columnIndex: colIdx) {
                    ...
                    // Unsafe lazy eval returned nil — fall back to Data-based path
                }
            } else if condition == nil, case .count(nil) = function {
                // COUNT(*) without column index and no condition — just count
                count += 1
                return
            }
            // Column extraction failed or no column index — fall back to Data-based path
            guard let base = ptr.baseAddress, ptr.count > 0 else { return }
            let recordData = Data(bytes: base, count: ptr.count)
        ...
        case .avg:
            ...
            if avgAllInts && !avgIntOverflow {
                return .double(Double(avgIntSum) / Double(avgCount))
            }
            // If overflow happened, avgSum has the partial double sum from overflow point onward
            let totalSum = avgIntOverflow ? avgSum : (avgSum + Double(avgIntSum))
            return .double(totalSum / Double(avgCount))
        case .min: return minMaxAllInts && minVal != .null ? .integer(minInt) : minVal
        case .max: return minMaxAllInts && maxVal != .null ? .integer(maxInt) : maxVal
        }
    }
    ...
    /// Zero-copy variant of evaluateConditionLazy: evaluates a WHERE condition against an UnsafeRawBufferPointer
    /// using positional column extraction only (v3 format). Returns nil if evaluation is not possible.
    private func evaluateConditionLazyUnsafe(_ condition: WhereCondition, ptr: UnsafeRawBufferPointer, columnIndexMap: [String: Int]) -> Bool? {
        guard ptr.count >= 4, let base = ptr.baseAddress else { return nil }
        // Only support positional v3 format for unsafe evaluation
        guard base.load(as: UInt8.self) == 0xFF,
              (base + 1).load(as: UInt8.self) == 0x03 else { return nil }

        let lookupColumn: (String) -> DBValue? = { col in
            guard let idx = columnIndexMap[col] else { return nil }
            return Row.extractColumnValueUnsafe(from: ptr, columnIndex: idx)
        }
        ...
        case .and(let subs):
            let ordered = subs.count <= 1 ? subs : subs.sorted { conditionCost($0) < conditionCost($1) }
            for sub in ordered {
                guard let result = evaluateConditionLazyUnsafe(sub, ptr: ptr, columnIndexMap: columnIndexMap) else { return nil }
                if !result { return false }
            }
            return true
        case .or(let subs):
    ...
    private func hashInnerJoinLazy(left: [(key: DBValue, data: Data)], right: [(key: DBValue, data: Data)], join: JoinClause, leftTable: String, leftSchema: CobaltTableSchema?, rightSchema: CobaltTableSchema?, strategy: JoinStrategy, limit: Int? = nil) -> [Row] {
    ...
    private func decodeRow(_ data: Data, schema: CobaltTableSchema?, neededColumns: Set<String>?) -> Row? {
    ...
    private func parallelTableScan(pageIDs: [Int], condition: WhereCondition?, limit: Int? = nil, neededColumns: Set<String>? = nil, projectColumns: Set<String>? = nil, schema: CobaltTableSchema? = nil, transactionContext: TransactionContext?) async throws -> [Row] {
        if let condition = condition {
