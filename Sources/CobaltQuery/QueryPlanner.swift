...
struct QueryPlanner: Sendable {
    ...
    private let planCache: CobaltLock<[PlanCacheKey: CachedPlan]>
    /// Generation counter: incremented on index/schema changes to invalidate cache
    private let cacheGeneration: CobaltLock<UInt64>
    /// Monotonic access counter for LRU eviction
    private let accessCounter: CobaltLock<UInt64>
    ...
    init(storageEngine: StorageEngine, indexManager: IndexManager, registry: TableRegistry, costWeights: CostModelWeights = .default) {
        ...
        self.planCache = CobaltLock([:])
        self.cacheGeneration = CobaltLock(0)
        self.accessCounter = CobaltLock(0)
