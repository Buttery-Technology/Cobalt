...
@testable import CobaltIndex
@testable import CobaltQuery

@Test func testQueryExecutorSelect() async throws {
    let path = NSTemporaryDirectory() + "cobalt_qe_\(UUID().uuidString).db"
...
@Test func testQueryExecutorUpdateDelete() async throws {
    let path = NSTemporaryDirectory() + "cobalt_qe_ud_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "data", columns: [
...
@Test func testConditionEvaluation() async throws {
    let path = NSTemporaryDirectory() + "cobalt_cond_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "test", columns: [
        CobaltColumn(name: "x", type: .integer),
        CobaltColumn(name: "y", type: .string),
    ])
    try await engine.createTable(schema)
...
@Test func testLikePatternMatching() async throws {
    let path = NSTemporaryDirectory() + "cobalt_like_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "strings", columns: [
        CobaltColumn(name: "val", type: .string),
    ])
    try await engine.createTable(schema)
...
@Test func testInCondition() async throws {
    let path = NSTemporaryDirectory() + "cobalt_in_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "items", columns: [
        CobaltColumn(name: "id", type: .integer), CobaltColumn(name: "name", type: .string),
    ])
    try await engine.createTable(schema)
...
@Test func testBetweenCondition() async throws {
    let path = NSTemporaryDirectory() + "cobalt_between_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "nums", columns: [
        CobaltColumn(name: "val", type: .integer),
    ])
    try await engine.createTable(schema)
...
@Test func testComplexNestedConditions() async throws {
    let path = NSTemporaryDirectory() + "cobalt_nested_cond_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "people", columns: [
        CobaltColumn(name: "name", type: .string),
        CobaltColumn(name: "age", type: .integer),
        CobaltColumn(name: "city", type: .string),
    ])
    try await engine.createTable(schema)
...
@Test func testNullSemanticsComprehensive() async throws {
    let path = NSTemporaryDirectory() + "cobalt_null_sem_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "nulltest", columns: [
        CobaltColumn(name: "a", type: .integer),
    ])
    try await engine.createTable(schema)
...
@Test func testUpdateNoMatchingRows() async throws {
    let path = NSTemporaryDirectory() + "cobalt_upd_none_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "data", columns: [
        CobaltColumn(name: "id", type: .integer), CobaltColumn(name: "v", type: .string),
    ])
    try await engine.createTable(schema)
...
@Test func testDeleteNoMatchingRows() async throws {
    let path = NSTemporaryDirectory() + "cobalt_del_none_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "data", columns: [
        CobaltColumn(name: "id", type: .integer),
    ])
    try await engine.createTable(schema)
...
@Test func testSelectFromEmptyTable() async throws {
    let path = NSTemporaryDirectory() + "cobalt_empty_sel_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "empty", columns: [
        CobaltColumn(name: "x", type: .integer),
    ])
    try await engine.createTable(schema)
...
@Test func testUpdateAllRows() async throws {
    let path = NSTemporaryDirectory() + "cobalt_upd_all_\(UUID().uuidString).db"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    let schema = CobaltTableSchema(name: "data", columns: [
        CobaltColumn(name: "id", type: .integer), CobaltColumn(name: "status", type: .string),
    ])
    try await engine.createTable(schema)
