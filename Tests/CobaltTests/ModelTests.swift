...
struct User: CobaltModel, Equatable {
...
struct Task_: CobaltModel, Equatable {
...
struct Event: CobaltModel, Equatable {
...
struct Document: CobaltModel, Equatable {
...
private func makeDB() async throws -> CobaltDatabase {
    let path = NSTemporaryDirectory() + "cobalt_model_\(UUID().uuidString).cobalt"
...
struct Widget: CobaltModel, Equatable {
    var id: String = UUID().uuidString
    var label: String
...
@Test func testConvenienceInit() async throws {
    let dbPath = CobaltConfiguration.databasePath(name: "default")
    try? FileManager.default.removeItem(atPath: dbPath)

    let db = try await CobaltDatabase()
...
@Test func testNamedInit() async throws {
    ...
    let db = try await CobaltDatabase(name: name)
...
@Test func testEncryptedConvenienceInit() async throws {
    ...
    let db = try await CobaltDatabase(name: name, encrypted: true)
    ...
    let keyPath = CobaltConfiguration.databasePath(name: name) + ".key"
    ...
    // Clean up
    let dbPath = CobaltConfiguration.databasePath(name: name)
    try? FileManager.default.removeItem(atPath: dbPath)
...
@Test func testEncryptedReopenWithKeyFile() async throws {
    ...
    // Open, write, close
    let db1 = try await CobaltDatabase(name: name, encrypted: true)
    let user = User(id: "u1", name: "Persist", age: 99)
    try await db1.save(user)
    try await db1.close()

    // Reopen same name — should reuse key file
    let db2 = try await CobaltDatabase(name: name, encrypted: true)
...
@Test func testGenerateKey() async throws {
    let key1 = CobaltConfiguration.generateKey()
    let key2 = CobaltConfiguration.generateKey()
...
@Test func testDefaultDirectory() async throws {
    let dir = CobaltConfiguration.defaultDirectory()
    #expect(!dir.isEmpty)
