...
@Test func testCobaltDatabaseCreateAndQuery() async throws {
    let path = NSTemporaryDirectory() + "cobalt_integration_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    // Create a table
    try await db.createTable(CobaltTableSchema(name: "users", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "name", type: .string, isNullable: false),
        CobaltColumn(name: "age", type: .integer),
    ]))
...
@Test func testKeyValueStore() async throws {
    let path = NSTemporaryDirectory() + "cobalt_kv_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testCodableStore() async throws {
    let path = NSTemporaryDirectory() + "cobalt_codable_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testEncryptedDatabase() async throws {
    let path = NSTemporaryDirectory() + "cobalt_encrypted_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    try await db.createTable(CobaltTableSchema(name: "secrets", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "value", type: .string),
    ]))
...
@Test func testBufferPoolStats() async throws {
    let path = NSTemporaryDirectory() + "cobalt_stats_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testBulkInsertAndQuery() async throws {
    let path = NSTemporaryDirectory() + "cobalt_bulk_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    try await db.createTable(CobaltTableSchema(name: "items", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "name", type: .string, isNullable: false),
        CobaltColumn(name: "score", type: .double),
    ]))
...
@Test func testIndexedQuery() async throws {
    let path = NSTemporaryDirectory() + "cobalt_indexed_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    try await db.createTable(CobaltTableSchema(name: "products", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "category", type: .string),
        CobaltColumn(name: "price", type: .double),
    ]))
...
@Test func testTableExistsAndListTables() async throws {
    let path = NSTemporaryDirectory() + "cobalt_tables_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    try await db.createTable(CobaltTableSchema(name: "posts", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
    ]))
...
@Test func testMultiPageTable() async throws {
    let path = NSTemporaryDirectory() + "cobalt_multipage_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    try await db.createTable(CobaltTableSchema(name: "large", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "payload", type: .string),
    ]))
...
@Test func testDataPersistsAcrossCloseAndReopen() async throws {
    let path = NSTemporaryDirectory() + "cobalt_persist_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    // Session 1: create table and insert data
    let db1 = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db1.createTable(CobaltTableSchema(name: "notes", columns: [
        .id("id"),
...
@Test func testEncryptedDataPersistsAcrossCloseAndReopen() async throws {
    let path = NSTemporaryDirectory() + "cobalt_enc_persist_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let key = Data((0..<32).map { UInt8($0) })

    // Session 1: write encrypted data
    let db1 = try await CobaltDatabase(configuration: CobaltConfiguration(
        path: path, encryptionKey: key
    ))
    try await db1.createTable(CobaltTableSchema(name: "secrets", columns: [
    ...
    let db2 = try await CobaltDatabase(configuration: CobaltConfiguration(
        path: path, encryptionKey: key
    ))
...
@Test func testAllWhereConditionTypes() async throws {
    let path = NSTemporaryDirectory() + "cobalt_where_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testNullHandling() async throws {
    let path = NSTemporaryDirectory() + "cobalt_null_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "nullable", columns: [
        .integer("id", nullable: false),
        .string("name"),
        .integer("score"),
    ]))
...
@Test func testNaNAndInfinityValues() async throws {
    let path = NSTemporaryDirectory() + "cobalt_nan_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testCodableStoreAutoIDAndOverwrite() async throws {
    let path = NSTemporaryDirectory() + "cobalt_codable_edge_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testCodableStoreComplexNestedTypes() async throws {
    let path = NSTemporaryDirectory() + "cobalt_nested_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testEmptyTableOperations() async throws {
    let path = NSTemporaryDirectory() + "cobalt_empty_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "empty", columns: [
...
@Test func testDropAndRecreateTable() async throws {
    let path = NSTemporaryDirectory() + "cobalt_recreate_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    // Create, insert, drop
    try await db.createTable(CobaltTableSchema(name: "temp", columns: [
        .integer("id", nullable: false), .string("val"),
    ]))
...
@Test func testDuplicateTableCreateThrows() async throws {
    let path = NSTemporaryDirectory() + "cobalt_dup_table_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testUnicodeAndSpecialCharacters() async throws {
    let path = NSTemporaryDirectory() + "cobalt_unicode_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "i18n", columns: [
        .id(), .string("text"),
    ]))
...
@Test func testTransactionCommit() async throws {
    let path = NSTemporaryDirectory() + "cobalt_tx_commit_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "accounts", columns: [
...
@Test func testTransactionRollbackOnError() async throws {
    let path = NSTemporaryDirectory() + "cobalt_tx_rollback_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "data", columns: [
        .id(), .integer("value"),
    ]))
...
@Test func testLargeScaleIndexedQuery() async throws {
    let path = NSTemporaryDirectory() + "cobalt_10k_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(
        path: path, bufferPoolCapacity: 4000
    ))

    try await db.createTable(CobaltTableSchema(name: "events", columns: [
        .integer("id", nullable: false),
        .string("type"),
        .double("timestamp"),
        .string("payload"),
...
@Test func testKVStoreAllValueTypes() async throws {
    let path = NSTemporaryDirectory() + "cobalt_kv_types_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testColumnProjectionEdgeCases() async throws {
    let path = NSTemporaryDirectory() + "cobalt_proj_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "wide", columns: [
...
@Test func testRetrieveFromNonExistentCollection() async throws {
    let path = NSTemporaryDirectory() + "cobalt_no_collection_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testMultipleTablesSimultaneously() async throws {
    let path = NSTemporaryDirectory() + "cobalt_multi_table_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    // Create 5 tables
    for i in 0..<5 {
        try await db.createTable(CobaltTableSchema(name: "table_\(i)", columns: [
...
@Test func testDeleteAllRows() async throws {
    let path = NSTemporaryDirectory() + "cobalt_del_all_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testRollbackRestoresOriginalData() async throws {
    let path = NSTemporaryDirectory() + "cobalt_rollback_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testRollbackAfterMultiPageModifications() async throws {
    let path = NSTemporaryDirectory() + "cobalt_rollback_multi_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWALCheckpointShrinks() async throws {
    let path = NSTemporaryDirectory() + "cobalt_ckpt_\(UUID().uuidString).cobalt"
...
@Test func testDataIntactAfterCheckpointAndReopen() async throws {
    let path = NSTemporaryDirectory() + "cobalt_ckpt_reopen_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db1 = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db1.createTable(CobaltTableSchema(name: "items", columns: [
        .id(), .integer("value"),
    ]))
    for i in 0..<20 {
...
@Test func testIndexPersistsAcrossRestart() async throws {
    let path = NSTemporaryDirectory() + "cobalt_idx_persist_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    // Session 1: create table, insert data, create index
    let db1 = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db1.createTable(CobaltTableSchema(name: "products", columns: [
        .integer("id", nullable: false),
        .string("category"),
        .double("price"),
    ]))
...
@Test func testManyTablesExceedOnePage() async throws {
    let path = NSTemporaryDirectory() + "cobalt_many_tables_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    for i in 0..<tableCount {
        try await db1.createTable(CobaltTableSchema(name: "table_\(String(format: "%03d", i))", columns: [
            .integer("id", nullable: false),
...
@Test func testFreeListPageReuse() async throws {
    let path = NSTemporaryDirectory() + "cobalt_freelist_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    try await db2.createTable(CobaltTableSchema(name: "reused", columns: [
        .integer("id", nullable: false), .string("data"),
    ]))
    for i in 0..<30 {
    ...
    // Verify data integrity
    let db3 = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
...
@Test func testFreeListSurvivesRestart() async throws {
    let path = NSTemporaryDirectory() + "cobalt_freelist_restart_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    // Session 1: create, populate, drop
    let db1 = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db1.createTable(CobaltTableSchema(name: "temp", columns: [
        .integer("id", nullable: false),
    ]))
    for i in 0..<20 {
    ...
    let db2 = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db2.createTable(CobaltTableSchema(name: "new_table", columns: [
        .integer("id", nullable: false),
    ]))
    for i in 0..<20 {
...
@Test func testSchemaFactoryMethods() async throws {
    let path = NSTemporaryDirectory() + "cobalt_factory_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testAggregateQueries() async throws {
    let path = NSTemporaryDirectory() + "cobalt_agg_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    try await db.createTable(CobaltTableSchema(name: "scores", columns: [
...
@Test func testStreamResults() async throws {
    let path = NSTemporaryDirectory() + "cobalt_stream_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testCompoundIndex() async throws {
    let path = NSTemporaryDirectory() + "cobalt_compound_idx_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    // Create a products table
    let schema = CobaltTableSchema(name: "products", columns: [
        CobaltColumn(name: "category", type: .string),
        CobaltColumn(name: "price", type: .double),
...
@Test func testCompoundIndexPersistence() async throws {
    let path = NSTemporaryDirectory() + "cobalt_compound_persist_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    // Create database, insert data, create compound index
    do {
        let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
        let schema = CobaltTableSchema(name: "orders", columns: [
            CobaltColumn(name: "customer", type: .string),
            CobaltColumn(name: "status", type: .string),
...
@Test func testMigrationAddColumn() async throws {
    let path = NSTemporaryDirectory() + "cobalt_migrate_add_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    // Create table with initial schema
    let schema = CobaltTableSchema(name: "items", columns: [
        CobaltColumn(name: "id", type: .string, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "name", type: .string),
    ])
    try await db.createTable(schema)
    ...
    try await db.migrate(table: "items", migrations: [
        Migration(version: 1, operations: [
            .addColumn(CobaltColumn(name: "price", type: .double, isNullable: true))
        ])
    ])
...
@Test func testMigrationAddColumnWithDefault() async throws {
    let path = NSTemporaryDirectory() + "cobalt_migrate_default_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    let schema = CobaltTableSchema(name: "settings", columns: [
        CobaltColumn(name: "id", type: .string, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "key", type: .string),
    ])
    try await db.createTable(schema)
    ...
    try await db.migrate(table: "settings", migrations: [
        Migration(version: 1, operations: [
            .addColumn(CobaltColumn(name: "value", type: .string, isNullable: true, defaultValue: .string("default")))
        ])
    ])
...
@Test func testMigrationDropColumn() async throws {
    let path = NSTemporaryDirectory() + "cobalt_migrate_drop_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    let schema = CobaltTableSchema(name: "profiles", columns: [
        CobaltColumn(name: "id", type: .string, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "name", type: .string),
        CobaltColumn(name: "legacy_field", type: .string),
    ])
    try await db.createTable(schema)
...
@Test func testMigrationRenameColumn() async throws {
    let path = NSTemporaryDirectory() + "cobalt_migrate_rename_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))

    let schema = CobaltTableSchema(name: "logs", columns: [
        CobaltColumn(name: "id", type: .string, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "msg", type: .string),
    ])
    try await db.createTable(schema)
...
@Test func testAutoMigrationNewOptionalProperty() async throws {
    let path = NSTemporaryDirectory() + "cobalt_automigrate_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    struct UserV1: CobaltModel, Equatable {
    ...
    struct UserV2: CobaltModel, Equatable {
        static let tableName = "evolving_users"
        var id: String = UUID().uuidString
        var name: String
        var age: Int
...
@Test func testWhereConditionColumnEquals() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_eq_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "t", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "name", type: .string),
    ]))
...
@Test func testWhereConditionColumnGreaterThan() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_gt_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(path: path))
    try await db.createTable(CobaltTableSchema(name: "t", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "val", type: .integer),
    ]))
...
@Test func testWhereConditionColumnLessThan() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_lt_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnNotEquals() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_neq_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnGreaterThanOrEqual() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_gte_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnLessThanOrEqual() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_lte_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnIsNull() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_null_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnIsNotNull() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_notnull_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnLike() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_like_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnIn() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_in_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionColumnBetween() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_btwn_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionAndCombinator() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_and_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testWhereConditionOrCombinator() async throws {
    let path = NSTemporaryDirectory() + "cobalt_wc_or_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testCountShorthand() async throws {
    let path = NSTemporaryDirectory() + "cobalt_count_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testExistsShorthand() async throws {
    let path = NSTemporaryDirectory() + "cobalt_exists_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testBatchInsert() async throws {
    let path = NSTemporaryDirectory() + "cobalt_batch_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testBatchInsertEmptyArray() async throws {
    let path = NSTemporaryDirectory() + "cobalt_batch_empty_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testListIndexes() async throws {
    let path = NSTemporaryDirectory() + "cobalt_idx_list_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testDropIndex() async throws {
    let path = NSTemporaryDirectory() + "cobalt_idx_drop_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testListCompoundIndex() async throws {
    let path = NSTemporaryDirectory() + "cobalt_idx_compound_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
...
@Test func testConcurrentReads() async throws {
    let path = NSTemporaryDirectory() + "cobalt_concurrent_rd_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    // 10 concurrent reader tasks — verify no crashes or data corruption
...
@Test func testSequentialWritesConcurrentReads() async throws {
    let path = NSTemporaryDirectory() + "cobalt_seqw_conr_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    // Now launch concurrent readers to verify data integrity
    try await withThrowingTaskGroup(of: Void.self) { group in
        for _ in 0..<5 {
            group.addTask {
                let rows = try await db.select(from: "data")
                ...
                let ids = Set(rows.compactMap { row -> Int64? in
                    if case .integer(let v) = row["id"] { return v }
                    return nil
                })
...
// MARK: - Write Stress Test

@Test func testBulkWriteStress() async throws {
    let path = NSTemporaryDirectory() + "cobalt_stress_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }

    let db = try await CobaltDatabase(configuration: CobaltConfiguration(
        path: path, bufferPoolCapacity: 500
    ))
    try await db.createTable(CobaltTableSchema(name: "stress", columns: [
        CobaltColumn(name: "id", type: .integer, isPrimaryKey: true, isNullable: false),
        CobaltColumn(name: "task_id", type: .integer),
    ]))

    // Insert 1000 rows sequentially — stress the page allocation and indexing
    for i in 0..<1000 {
        try await db.insert(into: "stress", values: [
            "id": .integer(Int64(i)),
            "task_id": .integer(Int64(i % 20)),
        ])
    }

    let all = try await db.select(from: "stress")
    #expect(all.count == 1000)

    // Verify no duplicate IDs
    let ids = Set(all.compactMap { row -> Int64? in
        if case .integer(let v) = row["id"] { return v }
        return nil
    })
    #expect(ids.count == 1000)

    // Verify query filtering works on large dataset
    let filtered = try await db.select(from: "stress",
        where: .equals(column: "task_id", value: .integer(5)))
    #expect(filtered.count == 50) // 1000/20
...
@Test func testTransactionVisibility() async throws {
    let path = NSTemporaryDirectory() + "cobalt_tx_vis_\(UUID().uuidString).cobalt"
    defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    // Insert inside transaction, verify visible within transaction
    try await db.transaction { tx in
        try await tx.insert(into: "items", values: [
            "id": .integer(1), "name": .string("in_tx"),
        ])
        // Should be visible within the transaction
        let rows = try await tx.select(from: "items")
        #expect(rows.count == 1)
        #expect(rows[0]["name"] == .string("in_tx"))
    }
